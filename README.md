...
I believe that data analysis is more than just drawing charts. My daily workflow focuses on a professional, scalable approach:
- **Database-First**: Building robust structures in PostgreSQL and DuckDB rather than processing massive data purely in memory.
- **Medallion Architecture (Bronze/Silver/Gold)**: Structuring data from raw API/File extraction (Bronze), through cleansing (Silver), to finalized Star Schema analytical layers (Gold).
...
- **Programming**: Python (Pandas, API Integration)
...
|---|---|---|---|
| [**01. Macroeconomic Analysis (GUS API)**](./01_Macroeconomic_Analysis_GUS) | Economics, Public Data | Python, REST API, JSON, PostgreSQL | In Progress |
